        ...
        <div class="profile-card">
            ...
            <div class="profile-card__main">
                <div class="profile-card__meta">
                    ...
                    <span class="status-pill {{ $periode->status === 'active' ? 'status-pill--published' : 'status-pill--draft' }}">{{ $periode->status === 'active' ? 'Aktif' : 'Arsip' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="acss-crud-body">
            <div class="acss-info-grid" style="display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 1rem; padding: 0;">
                <div class="acss-info-item">
                    ...
                    <strong>{{ $periode->tahunAkademik?->name ?? '-' }}</strong>
                </div>
                <div class="acss-info-item">
                    <span style="font-size: 1rem; font-weight: 600;">Semester</span>
                    <strong>{{ $periode->semester == 1 ? 'Ganjil' : 'Genap' }}</strong>
                </div>
                <div class="acss-info-item">
                    <span style="font-size: 1rem; font-weight: 600;">Masa Berlaku</span>
                    <strong>{{ $periode->tgl_mulai?->format('d/m/Y') }} - {{ $periode->tgl_selesai?->format('d/m/Y') }}</strong>
                </div>
                <div class="acss-info-item">
                    <span style="font-size: 1rem; font-weight: 600;">Nomor SK</span>
                    <strong>{{ $periode->sk_nomor ?: '-' }}</strong>
                </div>
                <div class="acss-info-item">
                    <span style="font-size: 1rem; font-weight: 600;">Link Dokumen SK</span>
                    <strong>
                        @if ($periode->sk_dokumen_url)
                            <a href="{{ $periode->sk_dokumen_url }}" target="_blank" class="text-link">Lihat dokumen</a>
                        @else
                            -
                        @endif
                    </strong>
        ...
        <div class="acss-modal__dialog acss-modal__dialog--large acss-modal__dialog--master">
            ...
            <form class="acss-form-stack-tight" method="POST" action="{{ route('kaprodi.periode.update', $periode) }}">
                ...
                <div class="form-actions form-actions--inline ">
                    <button type="button" class="button button--inline" data-periode-show-edit-close>Batal</button>
